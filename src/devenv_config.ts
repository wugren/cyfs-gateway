...
//
// 种子案例书，与 buckyos 仓库 src/devenv_config.ts 同源同步：SN（本仓库
// make_sn_config.ts）是该种子的消费方之一，buckyos 的 make_config.ts 是另一个。
...
//
// make_config.ts (buckyos) uses these seed facts to write the OOD/rootfs-owned
// local user and device environment. make_sn_config.ts (this repo) uses the same
